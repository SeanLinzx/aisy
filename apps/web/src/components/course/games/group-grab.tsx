...
export function GroupGrabGame() {
  ...
  return (
    <div className="space-y-5">
      {myGroup ? (
        <div className="kid-card-mint text-center py-6">
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="space-y-2">
        {draftGroups.map((g, i) => (
          <div key={i} className="flex flex-wrap gap-2 items-center">
            <input
              className="kid-input w-14 text-center"
              value={g.emoji}
              maxLength={2}
            ...
            />
            <input
              className="kid-input flex-1 min-w-[120px]"
              value={g.name}
              onChange={(e) => updateDraft(i, { name: e.target.value })}
              placeholder={`小组 ${i + 1} 名称，如「AI小侦探队」`}
            />
            <input
              type="number"
              min={1}
              className="kid-input w-20"
              value={g.capacity}
              onChange={(e) => updateDraft(i, { capacity: Math.max(1, Number(e.target.value) || 1) })}
              title="人数上限"
            />
            <span className="text-xs text-ink-soft shrink-0">人/组</span>
            {draftGroups.length > 2 && (
              <button type="button" onClick={() => removeDraft(i)} className="text-xs text-rose-500 font-bold px-2">
                删除
              </button>
            )}
          </div>
      ...
      <div className="flex flex-wrap gap-2">
        ...
        {!session?.active ? (
          <button onClick={onStart} disabled={busy || !session} className="kid-button-primary !py-2 !px-4 text-sm">
            🚀 开始抢组并推送给学生
          </button>
        ) : (
          <>
      ...
      {session && (
        <div className="space-y-3">
          ...
          <div className="grid sm:grid-cols-2 gap-3">
            {session.groups.map((g) => (
              <div key={g.id} className="rounded-2xl border-2 border-orange-100 bg-white p-3">
                <div className="flex items-center justify-between gap-2">
                  <div className="font-extrabold">{g.emoji} {g.name}</div>
                  <span className="tag">{g.members.length}/{g.capacity}</span>
                </div>
