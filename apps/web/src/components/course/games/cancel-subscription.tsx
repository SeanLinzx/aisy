...
function buildCancelSubPages(tx: (zh: string) => string) {
  const FEEDBACK = `
...
<div class="card">
  <div class="badge">${tx('👑 黄金会员')}</div>
  <div class="crown">👑</div>
  <h1>${tx('黄金会员 · 自动续费管理')}</h1>
  <p class="sub">${tx('您的黄金会员将于 3 天后自动续费 ¥198/年')}</p>
  <div class="perks">${tx('✨ 专属贴纸 · 🎮 无限关卡 · 🎁 每日礼包 · 📺 免广告')}</div>
  <button class="renew" onclick="__feedback('❌','${tx('点错啦！这是续费按钮，不是取消')}','#dc2626');__emit('page1_wrong')">${tx('🔥 立即续费 · 享 8 折特惠')}</button>
  <button class="cancel" onclick="__feedback('✅','${tx('找对啦！进入确认步骤…')}','#16a34a');__emit('page1_correct')">${tx('取消续费')}</button>
  <p class="hint">${tx('会员到期后将失去全部特权')}</p>
</div>
${FEEDBACK}</body></html>`;
...
<div class="dialog">
  ...
  <h2>${tx('确认取消自动续费？')}</h2>
  <p>${tx('取消后您将失去黄金会员全部特权，包括专属贴纸、无限关卡和每日礼包。')}</p>
  <button class="keep" onclick="__feedback('❌','${tx('点错啦！这是「继续会员」')}','#dc2626');__emit('page2_wrong')">${tx('💚 我再想想，继续会员')}</button>
  <button class="confirm" onclick="__feedback('🎉','${tx('太棒了！成功取消续费！')}','#16a34a');__emit('page2_correct')">${tx('确认取消')}</button>
</div>
${FEEDBACK}</body></html>`;

  return { PAGE1_HTML, PAGE2_HTML };
}

export function CancelSubscriptionGame() {
  const { tx } = useLanguage();
  const { PAGE1_HTML, PAGE2_HTML } = useMemo(() => buildCancelSubPages(tx), [tx]);
  const { data: session, loaded } = useCourseStreamChannel<CancelSubSession>('cancelSub', '/course/cancel-sub');
  ...
  if (!session?.active) {
    return (
      <div className="kid-card-orange text-center py-10">
        ...
        <div className="font-extrabold text-lg">{tx('老师还没开始「来取消续费吧」')}</div>
        <p className="text-sm text-ink-soft mt-1">{tx('请等老师在课堂控制台开启游戏并推送到你的屏幕。（会自动刷新）')}</p>
      </div>
    );
  }
    ...
    <div className="space-y-4">
      <div className="kid-card-purple">
        <p className="text-sm font-semibold text-ink-soft leading-relaxed">
          💳 <b>{tx('来取消续费吧！')}</b>{tx('这是一个故意设计得很难用的「黄金会员」页面。你的任务是：')}<b>{tx('找到并点击「取消续费」')}</b>{tx('，然后在弹窗里点击')}<b>{tx('「确认取消」')}</b>{tx('。小心那些又大又显眼的按钮，它们都在骗你点错！')}
        </p>
      </div>
      ...
      {stage === 'done' ? (
        <div className="kid-card-mint text-center py-8">
          <div className="text-5xl mb-3">🏆</div>
          <div className="font-extrabold text-xl text-emerald-800">{tx('挑战成功！')}</div>
          <p className="text-sm text-ink-soft mt-2">{tx('你从头到尾都选对了，没有被「垃圾交互」套路到！')}</p>
        </div>
      ) : (
        <div className="kid-card space-y-2">
          <div className="text-sm font-bold">
            {stage === 'page1' ? tx('👑 第 1 步：在黄金会员页面找到「取消续费」') : tx('⚠️ 第 2 步：在弹窗里点击「确认取消」')}
          </div>
    ...
    <div className="space-y-3">
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
        <div className="rounded-xl bg-white border-2 border-orange-100 p-3 text-center">
          ...
          <div className="text-[11px] text-ink-soft font-bold">{tx('参与人数')}</div>
        </div>
        <div className="rounded-xl bg-white border-2 border-emerald-100 p-3 text-center">
          ...
          <div className="text-[11px] text-ink-soft font-bold">{tx('已完成')}</div>
        </div>
        <div className="rounded-xl bg-white border-2 border-violet-100 p-3 text-center">
          ...
          <div className="text-[11px] text-ink-soft font-bold">{tx('全程正确率')}</div>
        </div>
        <div className="rounded-xl bg-white border-2 border-rose-100 p-3 text-center">
          ...
          <div className="text-[11px] text-ink-soft font-bold">{tx('总误点次数')}</div>
        </div>
      </div>

      <div className="text-xs text-ink-soft">
        {tx('界面①误点')} {totalP1} {tx('界面②误点')} {totalP2} {tx('全程零失误')} {perfect.length}{tx(' 人')}
      </div>
      ...
      {records.length > 0 ? (
        <div className="overflow-auto max-h-64 rounded-xl border-2 border-orange-100">
          <table className="w-full text-sm">
            <thead className="bg-orange-50 sticky top-0">
              <tr>
                <th className="text-left px-3 py-2 font-bold">{tx('小朋友')}</th>
                <th className="text-center px-2 py-2 font-bold">{tx('进度')}</th>
                <th className="text-center px-2 py-2 font-bold">{tx('①误点')}</th>
                <th className="text-center px-2 py-2 font-bold">{tx('②误点')}</th>
                <th className="text-center px-2 py-2 font-bold">{tx('结果')}</th>
              </tr>
            </thead>
            <tbody>
              {records.map((r) => {
                const perfectRun = r.completed && r.page1Errors === 0 && r.page2Errors === 0;
                const stageLabel = r.stage === 'page1' ? tx('界面①') : r.stage === 'page2' ? tx('界面②') : tx('完成');
                return (
                  <tr key={r.studentId} className="border-t border-orange-50">
                    ...
                    <td className="text-center px-2 py-2 text-xs font-bold">
                      {perfectRun ? tx('✅ 全程正确') : r.completed ? tx('⚠️ 有误点') : '…'}
                    </td>
                  </tr>
                );
              })}
        ...
        <p className="text-sm text-ink-soft">{tx('还没有小朋友开始，推送游戏后数据会实时更新。')}</p>
      )}
    </div>
  );
}
