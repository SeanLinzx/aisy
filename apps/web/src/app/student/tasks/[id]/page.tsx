  ...
  return (
    <div className="space-y-5 page-container--narrow">
      ...
      <div className="kid-card space-y-4">
        <h3 className="font-semibold">提交作品</h3>
        <div>
          <div className="text-sm font-semibold mb-2">选择素材（可多选）</div>
        ...
        <div>
          <div className="text-sm font-semibold mb-2">选择网页项目（可选）</div>
          <select className="kid-input" value={pickedProj} onChange={(e) => setPickedProj(e.target.value)}>
            <option value="">不附带网页</option>
            {projects.map(p => <option key={p.id} value={p.id}>{p.title}</option>)}
          </select>
        </div>
        <div>
          <div className="text-sm font-semibold mb-2">备注（可选）</div>
          <textarea className="kid-textarea" value={comment} onChange={(e) => setComment(e.target.value)} placeholder="想对老师说的话..." />
        </div>
        <button onClick={submit} className="kid-button-primary">提交作品</button>
