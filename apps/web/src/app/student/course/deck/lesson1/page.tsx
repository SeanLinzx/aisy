  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center gap-2 text-xs text-ink-soft font-bold">
        <Link href="/student/course" className="hover:text-brand">{tx('📚 课程模式')}</Link>
        <span>›</span>
        <Link href="/student/course/lesson1" className="hover:text-brand">{tx('第1课')}</Link>
        <span>›</span>
        <span>{tx('互动课件')}</span>
      </div>
      ...
      <header className="kid-card-orange">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-ink-soft mt-1">{tx('用 ← → 翻页，按 ↑ 或 Esc 显示老师工具栏（计时、跳转、全屏、批注）。')}</p>
          </div>
        </div>
      </header>
