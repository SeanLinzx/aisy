    ...
    <main className="relative min-h-screen overflow-hidden flex items-center justify-center px-6 py-12">
      ...
      <div className="relative max-w-4xl w-full text-center">
        ...
        <h1 className="font-display text-4xl md:text-6xl font-extrabold leading-tight">
          <span className="text-rainbow">AI 创作 + 网页训练营</span>
        </h1>
        <p className="mt-5 text-lg md:text-xl text-ink-soft font-semibold max-w-2xl mx-auto">
          这是属于小朋友的 AI 创作乐园 🎉
          <br />
          用 AI <span className="tag-yellow mx-1">写故事</span>
          <span className="tag-pink mx-1">画画</span>
          <span className="tag-sky mx-1">做视频</span>
          <span className="tag-mint mx-1">搭网页</span>
          ，发布到自己的作品主页！
        </p>

        <div className="mt-10 flex flex-wrap justify-center gap-4">
          <Link href={target} className="kid-button-primary text-lg px-7 py-4">
            {me ? `🚀 进入「${roleLabel(me.role)}」工作台` : '🚀 小朋友登录'}
          </Link>
        ...
        <div className="mt-14 grid sm:grid-cols-3 gap-5 text-left">
          {features.map((f, i) => (
            ...
            >
              ...
              <div className="mt-3 font-bold text-lg text-ink">{f.title}</div>
              <div className="text-sm text-ink-soft mt-1.5 leading-relaxed">{f.desc}</div>
            </div>
          ))}
        </div>

        <div className="mt-12 text-xs text-ink-soft/70">
          🦄 让创意飞起来 · 让 AI 帮你做超棒的作品
        </div>
      </div>
    </main>
  );
...
function roleLabel(role: string) {
  return ({ student: '学生', teacher: '老师', parent: '家长', admin: '管理员' } as any)[role] ?? role;
}
